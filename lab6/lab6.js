...
function testMail(telephone,mail) {
    var teleReg=/^[1][3,4,5,7,8][0-9]{9}$/;
    var isTelephone=teleReg.test(telephone.val());
    var mailReg = /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/;
    var isEmail=mailReg.test(mail.val());
    if (isTelephone&&!isEmail){
       console.log("The telephone is right, but the mail is wrong!");
    }
    else if (isTelephone&&isEmail){
        ...
        console.log("The mail is wrong and the telephone is wrong too.");
    }
}
...
function testRedundancy(str) {
    let strReg=/(\b\w+\b)(?:\s\1)/gi;
    let matches;
    let lastIndex=0;
    let set=new Set();
    while (lastIndex<str.length){
        matches=strReg.exec(str);

        if (matches !==null){
            set.add(matches[0]);
            strReg.lastIndex-=matches[1].length;
            lastIndex=strReg.lastIndex;
        }
        else break;
    }
    //阉割set
...
function testKeyBoard(wantInput, actualInput) {
     let wantArr=wantInput.split("");
     let setStr=new String();
     for (var i=0; i<wantArr.length;i++){
         if(actualInput.contains(String.valueOf(wantArr[i]))===false){
             if (setStr.contains(String.valueOf(wantArr[i]))===false){
                 setStr += wantArr[i];
             }
         }
     }
     let Set=setStr.split("");
     let jasonSet=JSON.stringify(Set);
     console.log(jasonSet);
}
...
function twoSum(nums, target) {
    //下面的实际上是网上的代码，思路就是对每响搜索前面的象中是否有满足题目条件的数字
        let map = new Map();
        let dif;
...
function lengthOfLongestSubstring(str) {
    ...
    for (let i = 0; i < str.length; i++) {
        char = str.charAt(i);
        index = empty.indexOf(char);
        if (index === -1) {
            empty += char;
            countNumber = countNumber < empty.length ? empty.length : countNumber;

        } else {
            empty = empty.substr(index + 1) + char;
        }
        map.set(empty,countNumber);
    }
